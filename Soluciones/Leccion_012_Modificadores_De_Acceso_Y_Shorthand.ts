...
// ¿Notaste que en la lección anterior repetimos `ancho` y `alto` tres veces?
...
class Cafetera {
  private cantidadActual: number = 0;

  constructor(public readonly capacidadMaxima: number){}

  servirCafe(cantidad:number): void {
    if(cantidad > 0 && cantidad <= this.cantidadActual){
      this.cantidadActual -= cantidad;
      console.log(`Te acabas de servir ${cantidad} de café. Mmm ¡qué rico!`)
    } else {
      console.log(`No te puedes servir tal cantidad de café. ¡No queda tanto!`)
    }
  }
