...
interface OrdenDeCompra {
  ...
  cliente: string,
  productos: Producto[],
  direccion: [string, number, string] // calle, numero, ciudad
}

function resumenDeOrden(orden: OrdenDeCompra): string {
  const sumaPrecios = orden.productos.reduce((acumulador, actual) => acumulador + actual.precio, 0)
  return `Resumen orden ${orden.id}: Cliente ${orden.cliente} | Estado: ${orden.estado} | Total: $${sumaPrecios}`;
}
